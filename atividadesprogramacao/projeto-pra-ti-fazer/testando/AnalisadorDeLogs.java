    ...
    public void processarRecursosGrandes() {
        try (Scanner scanner = new Scanner(new File(caminhoLog));
            ...
            while (scanner.hasNextLine()) {
            	...
            	Leitura entrada = new Leitura(scanner.nextLine());
                if (entrada.validade && entrada.foiSucesso() && entrada.tamanho > 2000) {
    ...
    public void processarNaoRespondidosNovembro() {
        try (Scanner scanner = new Scanner(new File(caminhoLog));
            ...
            while (scanner.hasNextLine()) {
                // EntradaDeLog entrada = LeitorDeLog.ler(scanner.nextLine());
                Leitura entrada = new Leitura(scanner.nextLine());
                if (entrada.validade && entrada.foiErroCliente() && entrada.ehDeNovembro2021()) {
    ...
    public void calcularPercentualPorSO() {
        ...
        try (Scanner scanner = new Scanner(new File(caminhoLog));
            ...
            while (scanner.hasNextLine()) {
                //EntradaDeLog entrada = LeitorDeLog.ler(scanner.nextLine());
                Leitura entrada = new Leitura(scanner.nextLine());
                if (entrada.validade && entrada.ehDoAno2021()) {
                    total++;
    ...
    public void calcularMediaPost2021() {
        ...
        try (Scanner scanner = new Scanner(new File(caminhoLog))) {
            while (scanner.hasNextLine()) {
                //EntradaDeLog entrada = LeitorDeLog.ler(scanner.nextLine());
            	Leitura entrada = new Leitura(scanner.nextLine());
                if (entrada.validade && entrada.ehDoAno2021() && entrada.foiSucesso() && entrada.metodo.equals("POST")) {
